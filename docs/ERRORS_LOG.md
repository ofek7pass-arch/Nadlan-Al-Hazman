...
### 27.06.2026 — קישורי מדלן בסיכום מחזירים 404
**תסמין:** סיכום יומי עם 2 דירות שהקישור שלהן מת (404)
**גורם:** פורמט URL שגוי — בניתי `madlan.co.il/listing/{id}` (יחיד). הנכון: `listings/{id}` (רבים). **כל** קישורי מדלן היו שבורים מההתחלה.
**טעות בתחקיר:** בבדיקה קודמת ראיתי 301 (תוספת `/`) ועצרתי — לא עקבתי עד ה-404 הסופי.
**פתרון:** `/listing/` → `/listings/` + `url=excluded.url` ב-upsert (לתקן קיימות) + **אימות קישורים לפני שליחת סיכום** (checkUrl, סינון 404/410).
**מניעה:** (1) תמיד לעקוב אחרי כל שרשרת ההפניות עד הדף הסופי. (2) לאמת קישורים יוצאים לפני כל התראה. (3) Railway חסום ע"י מדלן (403) — אימות מזהה רק 404 ודאי, לא מפיל 403.
